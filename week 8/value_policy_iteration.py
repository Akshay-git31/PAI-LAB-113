    ...
    while True:
        ...
        while True:
            ...
            for state in mdp.state_space:
                ...
                v = state_values[state]
                action = policy[state]
                state_values[state] = sum([p * (mdp.rewards[next_state] + gamma *
                    state_values[next_state])
                    for p, next_state in mdp.transitions[state][action]])
                delta = max(delta, abs(v - state_values[state]))
            if delta < 0.01:
        ...
        for state in mdp.state_space:
            action = policy.get(state, "TERMINAL")
            print(f"  State: {state}, Action: {action}, Value: {state_values[state]:.4f}")
        policy_stable = True
        for state in mdp.state_space:
            if state == mdp.goal or state == mdp.trap:
                continue
            ...
            policy[state] = max(mdp.action_space, key=lambda a: sum([p *
                (mdp.rewards[next_state] + gamma * state_values[next_state])
...
for state, action in policy_iteration_result.items():
    print(f"State: {state}, Action: {action}, Value: {policy_iteration_state_values[state]:.2f}")
